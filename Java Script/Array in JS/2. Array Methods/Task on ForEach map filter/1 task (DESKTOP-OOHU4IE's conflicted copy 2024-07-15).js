...
let arr =[1,3,4];
const result = arr.map(element => element *2);  //lambda or arrow function  this is  function used as arrow function 
...
let sum = 0;
num.forEach(cb => {
              if ( cb % 2 != 0){    // used to iterate over the array but do not have returntype or return anything
                sum += cb;
              }
});
console.log(sum);
